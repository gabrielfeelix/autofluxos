...
export function BarraLateral({ marca, identidadeNoCelular, voltar, itens, rodape, presenca, carregando = false }: {
  marca: ReactNode
  ...
  const conta = useRef<HTMLDialogElement>(null)
  const disponivel = presenca === 'disponivel'
  const rotuloConta = `Conta e perfil${presenca ? ` · ${disponivel ? 'Disponível' : 'Ausente'}` : ''}`
  ...
  const contaButton = (mobile = false) => (
    <button type="button" disabled={carregando} onClick={() => conta.current?.showModal()} aria-label={rotuloConta} title={rotuloConta}
      ...
      {!mobile && !recolhida && <span className="min-w-0"><span className="block text-xs font-semibold">Conta e perfil</span><span className="block text-[11px] text-dim">{presenca ? (disponivel ? 'Disponível' : 'Ausente') : 'Preferências e acesso'}</span></span>}
    </button>
  )
  return (
    <aside className={`flex shrink-0 flex-col border-line bg-panel md:border-r md:py-4 ${recolhida ? 'md:w-[68px] md:px-2.5' : 'md:w-[226px] md:px-3.5'}`}>
      ...
      <dialog ref={conta} aria-labelledby="titulo-conta" onClick={(event) => { if (event.target === event.currentTarget) conta.current?.close() }} className="fixed inset-0 m-auto w-[min(360px,calc(100%-32px))] rounded-2xl border border-line bg-panel p-5 text-ink shadow-xl backdrop:bg-black/30">
        <header className="mb-4 flex items-center justify-between"><h2 id="titulo-conta" className="text-base font-bold">Conta e perfil</h2><button type="button" aria-label="Fechar conta e perfil" onClick={() => conta.current?.close()} className="size-9 rounded-lg text-muted hover:bg-surface">×</button></header>
        {rodape}
