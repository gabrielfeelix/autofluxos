        ...
        {campos.length === 0 ? (
          ...
          <dl className="mt-3 grid gap-x-6 gap-y-3 sm:grid-cols-2">
            {campos.map(([chave, valor]) => (
              <div key={chave} className="rounded-xl border border-zinc-200 px-4 py-3 dark:border-zinc-800">
              ...
              return (
                <div key={mensagem.id} className={nossa ? 'flex justify-end' : 'flex justify-start'}>
                  <div className={`max-w-[85%] ${nossa ? 'text-right' : 'text-left'}`}>
                    ...
                    <span className="mt-1 block text-[11px] text-zinc-400" title={horaExata(mensagem.ts)}>
                      {nossa ? 'bot' : (lead.nome ?? 'cliente')} · {quando(mensagem.ts)}
                    </span>
                  </div>
                </div>
              )
