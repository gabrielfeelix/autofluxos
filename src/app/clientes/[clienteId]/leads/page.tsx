 ...
 *
 * **A ordem das duas partes é de propósito.** Primeiro a checagem de que o
 * cliente existe, que é uma busca por chave primária; só depois a consulta
 * pesada, dentro de um `<Suspense>`. Se a lista viesse antes, o Next já teria
 * começado a enviar a página quando descobrisse que o cliente não existe — e aí
 * "não encontrado" responde 200, porque status não se troca no meio do envio.
 * Está na doc desta versão, em `guides/streaming.md`.
 *
 * De quebra é o que faz a navegação parecer instantânea: o cabeçalho pinta na
 * hora e a tabela chega quando o banco responder.
 */
 ...
 * assunto" aqui seria o `core/` sabendo o nome de um cliente, só que na camada
 * de cima.
 */
